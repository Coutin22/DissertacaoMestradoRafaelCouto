...
#indice=1
  ...
  ggsave(paste0("Ajuste//2024//Sobreposto//",indice_ENSO[indice],".jpeg"), width = 15, height = 8, dpi = 300, units = "cm")
  ...
  ggsave(paste0("Ajuste//2024//Dispersão//",indice_ENSO[indice],".jpeg"), height=8.27,width=11.69)
  ...
  ggsave(paste0("Ajuste//2024//Dispersão//Overleaf//",indice_ENSO[indice],".pdf"), height=14,width=18)
...
pdf("Ajuste//2024//Dispersão//Overleaf//Ajuste_ENSO_2024_Geral.pdf",height=18,width=18)
...
write.xlsx(tabela_final, "Ajuste//2024//Modelos.xlsx")
#
...
Data <- format(seq(as.Date("2024-04-01"), as.Date("2024-12-01"), by = "month"), "%Y-%m-%d")

ONI_At <- c(0.760,0.326,-0.098,-0.425,-0.650,-0.865,-1.011,-0.965,-0.983)            

SOI_At <- EqSOI_At <- NINO12_At <- NINO3_At <- NINO4_At <- NINO34_At <- c(rep(NA,9))

ENSO_At <- cbind(Data,SOI_At,EqSOI_At,NINO12_At,NINO3_At,NINO4_At,NINO34_At,ONI_At)

colnames(ENSO_At) <- colnames(dados_ENSO)

dados_ENSO <- rbind(dados_ENSO,ENSO_At)

dados_ENSO <- dados_ENSO %>% mutate_at(vars(names(dados_ENSO)[-1]), as.numeric)
...
indice_ENSO <- c("SOI","Equatorial SOI","Niño 1+2","Niño 3","Niño 4","Niño 3.4","ONI")
# indice_ENSO <- colnames(dados_ENSO)[-1]

colnames(dados_ENSO) <- c("Data","SOI","EQSOI","NINO12","NINO3","NINO4","NINO34","ONI")
variaveis_ENSO <- colnames(dados_ENSO)[-1]

modelos <- read_excel("Ajuste/2024/Modelos.xlsx")
...
for (indice in 1:6) {
  ...
  reconstrucao <- dados[c(1120:1128),]
  
  for (i in 1:9) {
    #i=1
    reconstrucao[i,2] <- modelos[(2*indice-1),3] + (modelos[(2*indice),3] * as.numeric(reconstrucao[i,3]))     
  }
  
  #Salvando estatisticas (dados observados e reconstruidos)
  observado <- dados[-c(1120:1128),c(1,2)]
  ...
  write.xlsx(resumo_completo, paste0("Previsão//2024//Resumo//",indice_ENSO[indice],".xlsx"))
  ...
  dados_combinados <- dados_combinados[c((length(dados_combinados$Data)-179):length(dados_combinados$Data)),] 
  y <- colnames(dados_combinados)[2]
  
  tsibble_completo <- bind_rows(dados_combinados, tsibble(ultimo_mes, key = Serie))
  tsibble_completo$Data_aux <- c(1:171,171,172:180)
  ...
  ggplot(tsibble_completo, aes(x = Data_aux, color = Serie, linetype = Serie)) +
    ...
    scale_x_continuous(breaks = c(1,85,169), labels = c("Jan/2010","Jan/2017","Jan/2024")) +
  ...
  ggsave(paste0("Previsão//2024//",indice_ENSO[indice],".jpeg"), height=8.27,width=11.69)
  ...
  plot1 = ggplot(tsibble_completo, aes(x = Data_aux, color = Serie, linetype = Serie)) +
    ...
    scale_color_manual(values = c("Previsão" = "tomato", "Histórico" = "darkblue"),
                       labels = c("Previsão" = "Forecast", "Histórico" = "Historical")) +
  ...
  ggsave(paste0("Previsão//2024//Overleaf//",indice_ENSO[indice],".pdf"), height=14,width=18)
...
pdf("Previsão//2024//Overleaf//Artigo//Previsão_ENSO_2024_Geral.pdf",height=14,width=18)
# pdf("Previsão//2024//Overleaf//Previsão_ENSO_2024_Geral.pdf",height=14,width=18)
...
ENSO_final <- rbind(data.frame(dados_ENSO[c(1:1119),]),
...
write.xlsx(ENSO_final, "Previsão_ENSO_2024.xlsx")
...
ENSO <- read_excel("Previsão_ENSO_2024.xlsx")
...
write.xlsx(ENSO,"Previsão_ENSO_Completa_2024.xlsx")
